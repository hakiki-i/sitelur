    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card shadow-sm mt-4">
                <div class="card-header bg-success text-white">Status Produksi</div>
                <div class="card-body">
                    <div class="row g-4 mb-4">
                            ...
                            <div class="card shadow-sm h-100">
                                <div class="card-body d-flex align-items-center">
                                    <div>
                                        ...
                                        <div class="fs-4">{{ $produksiHarian }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="card shadow-sm h-100">
                                <div class="card-body d-flex align-items-center">
                                    <div>
                                        <div class="fw-bold text-secondary">Produksi Mingguan</div>
                                        <div class="fs-4">{{ $produksiMingguan }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="card shadow-sm h-100">
                                <div class="card-body d-flex align-items-center">
                                    <span class="fs-1 me-3 text-warning"><i class="fa-solid fa-calendar-days"></i></span>
                                    <div>
                                        <div class="fw-bold text-secondary">Produksi Bulanan</div>
                                        <div class="fs-4">{{ $produksiBulanan }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <!-- Tambahkan konten produksi lainnya di sini -->
                    <div class="mt-4">
                        <h5 class="fw-bold mb-3">Daftar Produksi Terbaru</h5>
                                ...
                                <tbody id="produksi-table-body">
                                    @foreach($listProduksi as $produksi)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge {{ $produksi->status == 'final' ? 'bg-success' : ($produksi->status == 'draft' ? 'bg-warning' : 'bg-secondary') }} text-white">
                                                    {{ ucfirst($produksi->status) }}
                                                </span>
                                            </td>
                                            <td>
                                                @if($produksi->status != 'final' && $produksi->status != 'approved' && $produksi->status != 'rejected')
                                                    <form method="POST" action="{{ route('produksi.validasi', $produksi->id) }}" style="display:inline;">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Setujui data produksi ini?')">Approved</button>
                                                    </form>
                                                    <form method="POST" action="{{ route('produksi.reject', $produksi->id) }}" style="display:inline; margin-left: 4px;">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Tolak data produksi ini?')">Rejected</button>
                                                    </form>
                    ...
                    <div class="d-flex justify-content-end align-items-center mt-2">
                        <div class="d-flex align-items-center gap-2">
                            <form method="get" class="d-flex align-items-center gap-1">
                                <label for="perPage" class="form-label mb-0 me-1 small">Tampil</label>
                                <select name="perPage" id="perPage" class="form-select form-select-sm d-inline-block w-auto" style="font-size:0.85em;min-width:60px;" onchange="this.form.submit()">
                                ...
                                <span class="ms-1 small">data</span>
                            </form>
                            <div>
