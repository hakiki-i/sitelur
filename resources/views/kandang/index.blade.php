
@extends('layouts.app')

@section('content')
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card shadow-sm mt-4">
                <div class="card-header bg-success text-white d-flex justify-content-between align-items-center">
                    <span>Data Kandang</span>
                    <a href="{{ route('kandang.create') }}" class="btn btn-primary">Tambah Kandang</a>
                </div>
                <div class="card-body">
                    <table class="table table-bordered table-striped">
                        <thead class="table-light">
                            <tr>
                                <th>No</th>
                                <th>Nama Kandang</th>
                                <th>Kapasitas</th>
                                <th>Keterangan</th>
                                <th>Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($kandang as $k)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $k->nama_kandang }}</td>
                                <td>{{ $k->jumlah_ayam }}</td>
                                <td>{{ $k->keterangan }}</td>
                                <td>
                                    <a href="{{ route('kandang.edit', $k->id_kandang ?? $k->id) }}" class="btn btn-warning btn-sm">Edit</a>
                                    <form action="{{ route('kandang.destroy', $k->id_kandang ?? $k->id) }}" method="POST" style="display:inline">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-danger btn-sm">Hapus</button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                    <div class="d-flex justify-content-end align-items-center mt-2">
                        <form method="get" class="me-2" style="font-size: 0.85em;">
                            <label for="perPage" class="me-1 mb-0">Tampil</label>
                            <select name="perPage" class="form-select form-select-sm d-inline w-auto" onchange="this.form.submit()">
                                <option value="10" {{ request('perPage', $perPage??25)==10?'selected':'' }}>10</option>
                                <option value="25" {{ request('perPage', $perPage??25)==25?'selected':'' }}>25</option>
                                <option value="50" {{ request('perPage', $perPage??25)==50?'selected':'' }}>50</option>
                                <option value="100" {{ request('perPage', $perPage??25)==100?'selected':'' }}>100</option>
                            </select>
                            <span class="ms-1">data</span>
                        </form>
                        {!! $kandang->withQueryString()->links('pagination::bootstrap-5') !!}
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
