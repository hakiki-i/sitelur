...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        ...
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Kandang</h2>
            <p class="text-sm text-gray-400 mt-0.5">Perbarui data kandang</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-blue-50 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-50 flex items-center gap-2">
            ...
            <span class="font-semibold text-gray-700">Form Edit Kandang</span>
        </div>
        <div class="p-6">
            <form action="{{ route('kandang.update', $kandang->id_kandang ?? $kandang->id) }}" method="POST" class="space-y-5">
                @csrf @method('PUT')
                <div>
                    ...
                    <input type="text" name="nama_kandang" id="nama_kandang" value="{{ $kandang->nama_kandang }}" required
                ...
                <div>
                    ...
                    <input type="number" name="jumlah_ayam" id="jumlah_ayam" value="{{ $kandang->jumlah_ayam }}" required
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all duration-200">
                </div>
                <div>
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3"
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent focus:bg-white transition-all duration-200 resize-none">{{ $kandang->keterangan }}</textarea>
                </div>
